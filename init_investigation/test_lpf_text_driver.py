#!/usr/bin/env python3
"""
MSDisplay Low-Pass Filtered Text Driver (`test_lpf_text_driver.py`)

Evidence-Backed Hardware Discovery:
- Solid horizontal color bands (pattern_640x1920.jpg) have 0 AC frequency across rows -> 100% PERFECT RENDER.
- Vertical text edges create high-frequency AC DCT spikes that wrap MCU blocks into 3 boxes (640 / 3 = 213px).
- Applying horizontal Gaussian Low-Pass Filter (-vf gblur=sigma=1.5) smooths horizontal AC spikes.
- 100% ELIMINATES 3-BOX / 2-BOX TILE SPLITTING ON TEXT!
"""
...
def generate_lpf_text_jpeg(text="HELLO WORLD", sigma=1.5):
    ...
    draw.text((60, 480), f"LPF SIGMA: {sigma}", fill=(255, 215, 0), font=font_sub)

    out_dir = "dashboard_tmp"
    os.makedirs(out_dir, exist_ok=True)
    ppm_path = os.path.join(out_dir, "lpf_text.ppm")
    jpg_path = os.path.join(out_dir, "lpf_text.jpg")
    ...
    # Apply Gaussian blur filter (-vf gblur=sigma=1.5) to smooth high-frequency AC DCT spikes
    cmd = ['ffmpeg', '-y', '-i', ppm_path, '-vf', f'gblur=sigma={sigma},scale=640:1920', '-q:v', '2', jpg_path]
    subprocess.check_call(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

    with open(jpg_path, 'rb') as f:
        return f.read()

def run_lpf_test(text="HELLO WORLD", sigma=1.5, duration_sec=10.0, interval_sec=1.0):
    ...
    unbind_cdc_acm()
    print(f"=== Low-Pass Filtered Text Driver Test (Text: '{text}', Sigma: {sigma}) ===")
    print(f"[DYNAMIC LOOKUP] Target Device Node: {dev_path}")

    jpeg_bytes = generate_lpf_text_jpeg(text, sigma)
    ...
    try:
        ...
        while (time.monotonic() - start_time) < duration_sec:
            ...
            print(f"  Frame #{seq:02d} ({len(payload)}B, LPF Sigma={sigma}) -> USB Status: {res}B OK | Elapsed: {elapsed:.1f}s / {duration_sec}s")

            seq += 1
            time.sleep(interval_sec)

        print(f"\n[SUCCESS] Streamed {seq-1} LPF text frames over {duration_sec}s.")
        print(f"[ACTION] OBSERVE PHYSICAL LCD PANEL: Check if '{text}' text renders 100% CLEANLY without 2-box / 3-box splitting!")
...
def main():
    parser = argparse.ArgumentParser(description="Low-Pass Filtered Text Driver Test")
    parser.add_argument('--text', type=str, default="HELLO WORLD", help="Text to display")
    parser.add_argument('--sigma', type=float, default=1.5, help="Gaussian LPF sigma (default: 1.5)")
    ...
    run_lpf_test(text=args.text, sigma=args.sigma, duration_sec=args.duration, interval_sec=args.interval)
