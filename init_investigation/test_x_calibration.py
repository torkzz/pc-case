#!/usr/bin/env python3
"""
2560-Wide X-Coordinate Column Calibration Driver (`test_x_calibration.py`)

Maps physical scanout column alignment across 2560x666 canvas:
- 10 columns (256px each), labeled 0 through 9 in 120pt bold font.
- Tells us instantly which numbers/columns map to physical screen!
"""
...
def generate_x_calib_jpeg():
    out_dir = "dashboard_tmp"
    os.makedirs(out_dir, exist_ok=True)
    ppm_path = os.path.join(out_dir, "x_calib.ppm")
    jpg_path = os.path.join(out_dir, "x_calib.jpg")
    ...
    draw.text((60, 25), "2560-WIDE X-COLUMN CALIBRATION (BOXES 0 - 9)", fill=(255, 255, 255), font=font_label)
...
def run_x_calib_test(duration_sec=10.0, interval_sec=1.0):
    ...
    unbind_cdc_acm()
    print("=== 2560-Wide X-Column Calibration Driver Test ===")
    print(f"[DYNAMIC LOOKUP] Target Device Node: {dev_path}")

    jpeg_bytes = generate_x_calib_jpeg()
    ...
    try:
        ...
        while (time.monotonic() - start_time) < duration_sec:
            ...
            print(f"  Frame #{seq:02d} ({len(payload)}B, Columns 0-9) -> USB Status: {res}B OK | Elapsed: {elapsed:.1f}s / {duration_sec}s")

            seq += 1
            time.sleep(interval_sec)

        print(f"\n[SUCCESS] Streamed {seq-1} X-calibration frames over {duration_sec}s.")
        print(f"[ACTION] OBSERVE PHYSICAL LCD PANEL: Tell me WHICH NUMBERS (0..9) appear on screen!")
...
def main():
    parser = argparse.ArgumentParser(description="2560-Wide X-Column Calibration Driver Test")
    ...
    run_x_calib_test(duration_sec=args.duration, interval_sec=args.interval)
