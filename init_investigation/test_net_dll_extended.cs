...
class Program
{
    static void Main(string[] args)
    {
        ...
        Console.WriteLine("=== EXTENDED .NET DLL COMMUNICATOR TEST ===");
        Console.WriteLine("Port: " + port);

        var comm = new DeviceCommunicator();
        comm.ErrorOccurred += (sender, err) => {
            Console.WriteLine("[DLL ERROR EVENT] " + err);
        };
        comm.DataReceived += (sender, data) => {
            Console.WriteLine("[DLL DATA EVENT] " + BitConverter.ToString(data));
        };
        ...
        if (ok)
        {
            ...
            try
            {
                ...
                {
                    Console.WriteLine("Handshake TIMEOUT after 10.0s.");
                }
            }
            catch (Exception ex)
            {
