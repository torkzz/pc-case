#!/usr/bin/env python3
"""
Simple Single-Text Minimal Driver Test (`test_simple_text.py`)

Minimal test case:
- Native resolution: 640 x 1920 (Portrait)
- Solid Dark Navy background
- ONE single text line ("HELLO WORLD")
...
def generate_simple_text_jpeg(text="HELLO WORLD"):
    ...
    try:
        font = ImageFont.truetype("/usr/share/fonts/TTF/DejaVuSans-Bold.ttf", 54)
    except Exception:
        font = ImageFont.load_default()

    # Draw Top Banner + Single Text String
    draw.rectangle([(0, 0), (WIDTH, 120)], fill=(0, 150, 214))
    draw.text((40, 35), "VMAX LCD TEST", fill=(255, 255, 255), font=font)

    draw.rectangle([(40, 300), (600, 500)], fill=(0, 230, 118))
    draw.text((80, 380), text, fill=(0, 0, 0), font=font)

    out_dir = "dashboard_tmp"
    os.makedirs(out_dir, exist_ok=True)
    ppm_path = os.path.join(out_dir, "simple_text.ppm")
    jpg_path = os.path.join(out_dir, "simple_text.jpg")
...
def run_simple_text_test(text="HELLO WORLD", duration_sec=10.0, interval_sec=1.0):
    ...
    unbind_cdc_acm()
    print(f"=== Simple Single-Text Minimal Driver Test (Text: '{text}') ===")
    ...
    try:
        ...
        print(f"\n[SUCCESS] Streamed {seq-1} simple text frames over {duration_sec}s.")
        print(f"[ACTION] OBSERVE PHYSICAL LCD PANEL: Check if '{text}' text box is clear, non-scrolling, and static.")
...
def main():
    parser = argparse.ArgumentParser(description="Simple Single-Text Minimal Driver Test")
    ...
    run_simple_text_test(text=args.text, duration_sec=args.duration, interval_sec=args.interval)
