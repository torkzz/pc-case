...
def create_huffman1_jpeg(text="HELLO WORLD"):
    ...
    draw.text((60, 1520), "640x1920 -huffman 1", fill=(255, 255, 255), font=font_lg)
    
    ppm_path = "/tmp/huffman1_test.ppm"
    jpg_path = "/tmp/huffman1_test.jpg"
...
def main():
    ...
    jpeg_bytes = create_huffman1_jpeg("HELLO WORLD")
    print(f"=== Perfect Huffman 1 Driver Test ({len(jpeg_bytes)} B JPEG) ===")
    
    # Inspect DHT length
    idx_dht = jpeg_bytes.find(b'\xff\xc4')
    if idx_dht != -1:
        dht_len = int.from_bytes(jpeg_bytes[idx_dht+2:idx_dht+4], 'big')
        print(f"DHT Marker len: {dht_len} bytes")
