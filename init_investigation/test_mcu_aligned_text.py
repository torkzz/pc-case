#!/usr/bin/env python3
"""
MCU Block Aligned & Raw Frame Driver (`test_mcu_aligned_text.py`)

Fixes 3-part image splitting:
- Enforces strict 8x8 / 16x16 JPEG MCU Macroblock boundary alignment (640x1920 = 80x240 MCU blocks).
- Disables restart markers and progressive scan passes.
...
def generate_mcu_aligned_jpeg(text="HELLO WORLD"):
    ...
    # Draw Top Banner + Single Text String aligned to MCU 16x16 grid
    draw.rectangle([(0, 0), (WIDTH, 128)], fill=(0, 150, 214)) # 128 = 16 * 8
    draw.text((40, 32), "VMAX LCD TEST", fill=(255, 255, 255), font=font)

    draw.rectangle([(32, 256), (608, 512)], fill=(0, 230, 118)) # 256..512 aligned to 16px
    draw.text((64, 352), text, fill=(0, 0, 0), font=font)

    out_dir = "dashboard_tmp"
    os.makedirs(out_dir, exist_ok=True)
    ppm_path = os.path.join(out_dir, "mcu_text.ppm")
    jpg_path = os.path.join(out_dir, "mcu_text.jpg")
    ...
    # Force strict 8x8 / 16x16 MCU block single-pass encoding
...
def run_mcu_test(text="HELLO WORLD", duration_sec=10.0, interval_sec=1.0):
    ...
    unbind_cdc_acm()
    print(f"=== MCU Block Aligned Minimal Driver Test (Text: '{text}') ===")
    print(f"[DYNAMIC LOOKUP] Target Device Node: {dev_path}")

    jpeg_bytes = generate_mcu_aligned_jpeg(text)
    ...
    try:
        ...
        print(f"\n[SUCCESS] Streamed {seq-1} MCU-aligned text frames over {duration_sec}s.")
        print(f"[ACTION] OBSERVE PHYSICAL LCD PANEL: Check if 3-part image splitting is eliminated.")
...
def main():
    parser = argparse.ArgumentParser(description="MCU Block Aligned Minimal Driver Test")
    ...
    run_mcu_test(text=args.text, duration_sec=args.duration, interval_sec=args.interval)
