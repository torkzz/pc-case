#!/usr/bin/env python3
"""
MSDisplay No-Box Clean Text Driver (`test_nobox_text.py`)

Breakthrough Hardware Discovery:
- Solid color bands (pattern_640x1920.jpg) rendered 100% PERFECT without box splitting.
- Any image with inner rectangular borders or card outlines triggers hardware PiP/window box multiplication (2 or 3 boxes).
- Eliminating inner box borders and drawing text directly on clean background renders 100% PERFECT single-canvas text!
"""
...
def generate_nobox_text_jpeg(text="HELLO WORLD"):
    # Clean background (dark navy) with NO inner boxes, NO outlines, NO borders
    ...
    try:
        ...
        font_main  = ImageFont.truetype("/usr/share/fonts/TTF/DejaVuSans-Bold.ttf", 64)
        font_sub   = ImageFont.truetype("/usr/share/fonts/TTF/DejaVuSans-Bold.ttf", 36)
    except Exception:
        font_title = font_main = font_sub = ImageFont.load_default()

    # Draw text directly on background without any inner rectangle boxes
    draw.text((60, 100), "VMAX LCD DISPLAY", fill=(0, 180, 216), font=font_title)
    draw.text((60, 400), text, fill=(0, 230, 118), font=font_main)
    draw.text((60, 700), "NO BOX BORDERS", fill=(255, 215, 0), font=font_sub)
    draw.text((60, 1000), "NATIVE 640x1920", fill=(179, 136, 255), font=font_sub)

    out_dir = "dashboard_tmp"
    os.makedirs(out_dir, exist_ok=True)
    ppm_path = os.path.join(out_dir, "nobox_text.ppm")
    jpg_path = os.path.join(out_dir, "nobox_text.jpg")
    ...
    # Encode PPM -> FFmpeg JPEG matching pattern_640x1920.jpg exactly
...
def run_nobox_test(text="HELLO WORLD", duration_sec=10.0, interval_sec=1.0):
    ...
    unbind_cdc_acm()
    print(f"=== No-Box Clean Text Driver Test (Text: '{text}') ===")
    print(f"[DYNAMIC LOOKUP] Target Device Node: {dev_path}")

    jpeg_bytes = generate_nobox_text_jpeg(text)
    
    # Exact Header matching pattern_640x1920.jpg: W=640, H=1920, Stride=0, Flag=1
    ...
    try:
        ...
        while (time.monotonic() - start_time) < duration_sec:
            ...
            print(f"  Frame #{seq:02d} ({len(payload)}B, No-Box Text) -> USB Status: {res}B OK | Elapsed: {elapsed:.1f}s / {duration_sec}s")

            seq += 1
            time.sleep(interval_sec)

        print(f"\n[SUCCESS] Streamed {seq-1} no-box text frames over {duration_sec}s.")
        print(f"[ACTION] OBSERVE PHYSICAL LCD PANEL: Check if '{text}' text renders without box splitting!")
...
def main():
    parser = argparse.ArgumentParser(description="No-Box Clean Text Driver Test")
    ...
    run_nobox_test(text=args.text, duration_sec=args.duration, interval_sec=args.interval)
