...
def create_exact_header_injected_jpeg(text="HELLO WORLD"):
    # Load exact 204B header from pattern_640x1920.jpg
    with open('portrait_patterns/pattern_640x1920.jpg', 'rb') as f:
        baseline_bytes = f.read()
        
    exact_hdr = baseline_bytes[:0x00CC]
    
    # Generate PPM image in RAM
    ...
    draw.text((60, 80), "EXACT HEADER MONITOR", fill=(0, 200, 255), font=font_md)
    draw.text((60, 200), text, fill=(255, 255, 255), font=font_lg)
    
    draw.rectangle([30, 480, 610, 880], fill=(20, 60, 40), outline=(0, 255, 120), width=3)
    draw.text((60, 520), "BASELINE HEADER", fill=(0, 255, 120), font=font_md)
    draw.text((60, 640), "EXACT 204B INJECTED", fill=(255, 255, 255), font=font_lg)
    ...
    draw.text((60, 1080), "HARDWARE MATCHED", fill=(255, 255, 255), font=font_lg)
    ...
    ppm_path = "/tmp/injected.ppm"
    jpg_path = "/tmp/injected_raw.jpg"
    ...
    # Find SOS marker (0xFF 0xDA) in generated JPEG
    sos_idx = generated_bytes.find(b'\xff\xda')
    sos_len = int.from_bytes(generated_bytes[sos_idx+2:sos_idx+4], 'big')
    scan_data = generated_bytes[sos_idx+2+sos_len:]
    
    # Combine exact baseline header (0x00CC bytes) with scan data
    injected_jpeg = exact_hdr + scan_data
    return injected_jpeg
...
def main():
    ...
    jpeg_bytes = create_exact_header_injected_jpeg("HELLO WORLD")
    print(f"=== Exact Header Injection Driver Test ({len(jpeg_bytes)} B JPEG) ===")
